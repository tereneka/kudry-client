...
export default function PhotoPage({ folderPath, pageNumber, photoPathList }: Props) {
    ...
    const isPageLoading = photoState.isLoading[folderPath]
        &&
    ...
    const isPageError = photoState.isError[folderPath]
        &&
    ...
    ${pageNumber <= photoState.openedPages[folderPath] && !isPageLoading && !isPageError
            ? 'photo__grid_opened' : ''}`;
